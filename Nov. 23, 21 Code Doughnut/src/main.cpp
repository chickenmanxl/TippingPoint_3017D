...
while(dist()>100)
  ...
  liftR.setVelocity(90, pct); //Lift speed
  ...
  liftL.spinFor(reverse, 5, rev, true); //Lift up
  wait(.5, seconds);
  liftR.spinFor(reverse, 5, rev, true); //Lift 2 up
...
void usercontrol(void) {
 while(1){
  ...
  expandL.stop(); //stops expansion
    ...
    while(Controller1.ButtonL1.pressing()) //lift up
    {
    ...
    {
      liftR.spin(reverse,60,pct); //motor reverse
    }

    while(Controller1.ButtonR2.pressing()) //lift down
    {
      liftR.spin(fwd,80,pct); //motor forward
    }
